...
<body class="flex h-screen overflow-hidden">
    ...
    <div id="main-content">
        ...
        <div id="page-content">
            <div class="page-hdr">
                <div class="page-hdr-top">
                    <div>
                        ...
                        <div class="pg-title">HARI <span>LIBUR NASIONAL</span></div>
                        <div class="pg-sub">Kelola master hari libur nasional & cuti bersama. Hari libur tetap dan
                            berbasis Paskah dibuat otomatis — sisanya cukup diinput sekali saat SKB terbit.</div>
                    </div>
                    <div class="pg-actions">
                        <select id="filterTahun" class="filter-select" onchange="onTahunChange()"
                            style="margin-right:8px;"></select>
                        <button class="btn-outline" onclick="generateTahunBerjalan()">
                            ⚡ Generate Otomatis
                        </button>
            ...
            <div id="kelengkapanAlert" style="display:none; margin-bottom:14px;"></div>
            ...
            <div class="section-card" style="margin-bottom:14px;">
                <div class="filter-bar">
                    <div class="filter-search">
                        ...
                        <input type="text" id="searchInput" placeholder="Cari nama hari libur..."
                    ...
                    <select id="filterKategori" class="filter-select" onchange="onFilterChange()">
                        ...
                        <option value="MASEHI_TETAP">Masehi Tetap (Auto)</option>
                        <option value="PASKAH">Paskah (Auto)</option>
    ...
    <div id="formModalOverlay" class="modal-overlay" onclick="closeFormModalOutside(event)">
        <div class="modal-box" onclick="event.stopPropagation()">
            <div class="form-modal-header">
                <div class="modal-title" id="formModalTitle">Tambah Hari Libur</div>
                <div class="pg-sub" style="margin:0;">Untuk hari libur berbasis SKB (Hijriah/Imlek/Nyepi/Waisak),
                    input begitu tanggal resmi terbit.</div>
            </div>
            <div class="form-modal-body">
                <div class="form-grid">
                    ...
                    <div class="form-group span-2"><label class="form-label">Nama Hari Libur</label>
                        <input type="text" id="fNamaLibur" class="form-input"
                            placeholder="Contoh: Hari Raya Idul Fitri 1449 H" />
                    </div>
                    <div class="form-group"><label class="form-label">Kategori</label>
                        <select id="fKategori" class="form-input">
                            <option value="HIJRIAH">Hijriah</option>
                            <option value="IMLEK">Imlek</option>
                            <option value="NYEPI">Nyepi</option>
                            <option value="WAISAK">Waisak</option>
                            <option value="MASEHI_TETAP">Masehi Tetap</option>
                            <option value="PASKAH">Paskah</option>
                            <option value="LAINNYA">Lainnya</option>
                        </select>
                    </div>
                    <div class="form-group span-2"><label class="form-label">Keterangan (opsional)</label>
    ...
    <script>
        const DATA_ENDPOINT = "{{ route('hari-libur.data') }}";
        const STORE_ENDPOINT = "{{ route('hari-libur.store') }}";
        const GENERATE_ENDPOINT = "{{ route('hari-libur.generate') }}";
        const BASE_ENDPOINT = "{{ url('/hari-libur') }}";
        const CSRF_TOKEN = "{{ csrf_token() }}";

        const KATEGORI_LABEL = {
            MASEHI_TETAP: 'Masehi Tetap',
            PASKAH: 'Paskah',
            HIJRIAH: 'Hijriah',
            IMLEK: 'Imlek',
            NYEPI: 'Nyepi',
        ...
        function populateTahunSelect() {
            ...
            for (let y = now - 2; y <= now + 3; y++) {
                ...
                o.value = y;
                o.textContent = y;
                if (y === state.tahun) o.selected = true;
                sel.appendChild(o);
            }
        }
        ...
        function onFilterChange() {
            state.jenis = document.getElementById('filterJenis').value;
        ...
        function renderKelengkapanAlert(kelengkapan) {
            const box = document.getElementById('kelengkapanAlert');
            if (!kelengkapan || kelengkapan.lengkap) {
                box.style.display = 'none';
                box.innerHTML = '';
                return;
            }
            const labels = kelengkapan.kategori_belum_diinput.map(k => KATEGORI_LABEL[k] || k).join(', ');
            box.innerHTML = `
                <div class="section-card" style="border-left:4px solid #D0021B; background:#FFF7F7;">
                    <strong style="color:#D0021B;">⚠ Data tahun ${kelengkapan.tahun} belum lengkap.</strong>
                    <span style="color:#334155;"> Kategori berikut belum diinput (menunggu SKB / penanggalan resmi): <strong>${escapeHtml(labels)}</strong>.</span>
                </div>`;
        ...
        function renderTable(rows) {
            ...
            if (!rows || rows.length === 0) {
                tbody.innerHTML =
                    `<tr><td colspan="6"><div class="empty-state"><div class="empty-state-title">Belum ada data untuk tahun ${state.tahun}</div></div></td></tr>`;
                return;
            }
            tbody.innerHTML = rows.map(row => `
                <tr>
                    <td style="font-weight:600;">${formatDate(row.tanggal)}</td>
                    <td>${escapeHtml(row.nama_libur)}</td>
                    <td><span class="status-pill ${row.jenis === 'CUTI_BERSAMA' ? 'sp-amber' : 'sp-green'}">${row.jenis === 'CUTI_BERSAMA' ? 'Cuti Bersama' : 'Libur Nasional'}</span></td>
                    <td>${KATEGORI_LABEL[row.kategori] || row.kategori}</td>
                    <td><span class="status-pill ${row.sumber === 'AUTO' ? 'sp-green' : 'sp-amber'}">${row.sumber === 'AUTO' ? 'Otomatis' : 'Manual'}</span></td>
                    <td style="text-align:center; white-space:nowrap;">
                        <button class="btn-outline" style="padding:5px 8px;" onclick='openFormModal(${JSON.stringify(row).replace(/'/g, "&#39;")})'>Edit</button>
                        <button class="btn-outline" style="padding:5px 8px; color:#D0021B; border-color:rgba(208,2,27,0.25);" onclick="deleteData(${row.id}, ${JSON.stringify(row.nama_libur)})">Hapus</button>
                    </td>
                </tr>
            `).join('');
        }
        ...
        async function loadData() {
            ...
            try {
                ...
                document.getElementById('dataSummary').innerHTML =
                    `<strong>${json.data.length}</strong> hari libur pada tahun ${state.tahun}`;
                renderKelengkapanAlert(json.kelengkapan);
            } catch (e) {
        ...
        async function generateTahunBerjalan() {
            if (!confirm(`Generate hari libur otomatis (Masehi tetap & Paskah) untuk tahun ${state.tahun}?`)) return;
            try {
                ...
                showToast(e.message || 'Gagal generate hari libur.', 'error');
            }
        }

        function openFormModal(row = null) {
            currentEditId = row ? row.id : null;
            document.getElementById('formModalTitle').textContent = row ? 'Edit Hari Libur' : 'Tambah Hari Libur';
            document.getElementById('fTanggal').value = row ? row.tanggal : '';
            document.getElementById('fNamaLibur').value = row ? row.nama_libur : '';
            document.getElementById('fJenis').value = row ? row.jenis : 'LIBUR_NASIONAL';
            document.getElementById('fKategori').value = row ? row.kategori : 'HIJRIAH';
            document.getElementById('fKeterangan').value = row ? (row.keterangan || '') : '';
        ...
        async function submitForm() {
            ...
            const payload = {
                ...
                nama_libur: document.getElementById('fNamaLibur').value,
